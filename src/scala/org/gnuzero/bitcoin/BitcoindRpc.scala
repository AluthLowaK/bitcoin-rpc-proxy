...
object BitcoinRPC {
  ...
  def getnewaddressForAccount(account: String)(implicit jsonrpc: JSONRPC) = {
    val json = jStringInjArray(account)
    jsonrpc.doRequest[String](JSONRPCRequest("t0", "getnewaddress", json), jsonToString)
  }
